...
class DetailPetUser : AppCompatActivity() {
    private val viewModel by viewModels<DetailPetUserViewModel> {
        ViewModelFactory.getInstance(this)
    }

    lateinit var binding: ActivityDetailPetUserBinding
    lateinit var progressDialog: ProgressDialog
    var mypetmodel: MypetResponse? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = DataBindingUtil.setContentView(this, R.layout.activity_detail_pet_user)
        ...
        binding.btnDelete.setOnClickListener {
            viewModel.delete_mypet(mypetmodel!!.petId!!).observe(this) { result ->
                when (result) {
                    is UiState.Loading -> {
                        showLoading(true)
                    }
        ...
        if (mypetmodel?.status == 0){
            binding.btnUpload.setOnClickListener {
                showcustomdialog(mypetmodel!!.petId!!)
            }

            val color = Color.parseColor("#65B741") // Ini adalah kode warna hijau
            val textColor = Color.parseColor("#FFFFFF")
            binding.btnUpload.backgroundTintList = ColorStateList.valueOf(color)
            binding.btnUpload.setTextColor(textColor)
        }else{
            val color = Color.parseColor("#FFFFFF") // Ini adalah kode warna hijau
            val textColor = Color.parseColor("#FF8080")
            binding.btnUpload.backgroundTintList = ColorStateList.valueOf(color)
            binding.btnUpload.setTextColor(textColor)
        }
